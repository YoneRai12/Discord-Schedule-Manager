...
足りない必須項目があれば「開始日時が足りません」のように項目名を返します。会議名だけ無い場合は`会議 7/21 19:00`のような仮名を付け、黄色い確認画面で自動設定と明示します。URL未定の確認画面では「今いるDiscord VC」「Google Meet / 外部URL」「未定で登録」をボタンで選びます。`/meeting create`では`voice_channel`からDiscord VCを選択でき、`url`と両方を省略すれば未定で作成できます。
...
この機能は初期状態ではOFFです。管理者が手動で開始し、同じVCにいる全員がDiscord上の「同意する」を押すまでBotはVCへ接続せず、音声を1バイトも保存しません。途中から人が入った場合は全員分の収録を一時停止し、その人の同意後に再開します。現在の参加者なら誰でも停止できます。
